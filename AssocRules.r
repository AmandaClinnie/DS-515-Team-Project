...
train_tran <-read.transactions('aug_train3.csv', sep =",", rm.duplicates = TRUE)
summary(train_tran)
...
itemFrequencyPlot(train_tran, support = 0.1, col =brewer.pal(8, 'Pastel2'))

# Show top 20 frequency items
itemFrequencyPlot(train_tran, topN= 20, col =brewer.pal(20, 'Pastel2')) # STEM, Remain, no_enrollment, Has Rel exp, Male, Grad, PVT Ltd
...
inspect(subsettrules) #22 rules with (male, pvt ltd, HRE, STEM, no Enr, Grad)
...
# at Pvt Ltd company, DS has rel. exp., STEM, Male, Grad, no Enr
# Subset with 1 item 
subsettrules <- subset(train_rules, items %ain% c("Remain"))
